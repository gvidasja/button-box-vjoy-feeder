...
const byte encoderSkip[] = {1, 1, 0, 0};
const char* encoderNames[8] = {
  "R1-",
  "R1+",
  "R2-",
  "R2+",
...
void send(bool msg, int button) {
  Serial.print(msg); Serial.println(button);
}
...
void printMatrix() {
  ...
  for (int i = 0; i < encCount; i += 2) {
    if ((encoders[i] != prevEncoders[i])) {
      ...
      encoderSent[i / 2] = (encoderSkip[i / 2] == 0) ? 0 : (encoderSent[i / 2] + 1) % encoderSkip[i / 2];
    }
  }
}
